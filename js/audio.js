(function attachAudio(root, factory) {
  ...
  class AudioManager {
    ...
    async playMain(style, options = {}) {
      return this._play(options, (context, start) => {
        if (style === "bright-bell") {
          this._tone(context, 659.25, start, 0.1, 0.72, 0.38, "triangle");
          this._tone(context, 987.77, start + 0.015, 0.07, 0.4, 0.42, "sine");
          this._tone(context, 1318.51, start + 0.025, 0.055, 0.2, 0.34, "sine");
          return;
        }

        if (style === "soft-chime") {
          this._tone(context, 523.25, start, 0.12, 0.42, 0.3, "sine");
          this._tone(context, 659.25, start + 0.1, 0.11, 0.34, 0.31, "sine");
          this._tone(context, 783.99, start + 0.2, 0.09, 0.26, 0.3, "sine");
          return;
        }

        // Glass Ping: a strong, bright transient with several quickly decaying partials.
        this._tone(context, 783.99, start, 0.055, 0.28, 0.38, "triangle");
        this._tone(context, 1046.5, start, 0.07, 0.78, 0.52, "sine");
        this._tone(context, 1567.98, start + 0.006, 0.045, 0.4, 0.42, "sine");
        this._tone(context, 2093, start + 0.01, 0.035, 0.18, 0.31, "sine");
      });
    }
    ...
    async playSecondary(style, options = {}) {
      return this._play(options, (context, start) => {
        if (style === "signal-drop") {
          this._sweep(context, 740, 430, start, 0.19, 0.58, 0.16, "triangle");
          this._sweep(context, 620, 350, start + 0.25, 0.17, 0.5, 0.14, "triangle");
          return;
        }

        if (style === "wood-block") {
          this._tone(context, 470, start, 0.035, 0.76, 0.07, "square");
          this._tone(context, 335, start + 0.12, 0.04, 0.68, 0.08, "square");
          this._tone(context, 235, start + 0.13, 0.045, 0.34, 0.08, "triangle");
          return;
        }

        // Low Double Tap stays well below the bright main ping so it is easy to identify.
        this._tone(context, 293.66, start, 0.08, 0.72, 0.13, "triangle");
        this._tone(context, 220, start, 0.065, 0.34, 0.12, "sine");
        this._tone(context, 329.63, start + 0.21, 0.085, 0.78, 0.14, "triangle");
        this._tone(context, 246.94, start + 0.21, 0.07, 0.36, 0.13, "sine");
      });
    }
    ...
    async playCompletion(options = {}) {
      return this._play(options, (context, start) => {
        ...
        notes.forEach((frequency, index) => {
          ...
          this._tone(context, frequency * 2, noteStart + 0.01, 0.04, 0.12, 0.2, "sine");
        });
      });
    }
    ...
    async _play(options, buildSound) {
      ...
      try {
        ...
        buildSound(context, context.currentTime + 0.025 + delaySeconds);
        return true;
      } catch (error) {
    ...
    _tone(context, frequency, start, sustain, peak, release, type) {
      ...
      this._trackSource(oscillator, gain);
      oscillator.start(start);
      oscillator.stop(end + 0.035);
    }

    _sweep(context, fromFrequency, toFrequency, start, duration, peak, release, type) {
      const oscillator = context.createOscillator();
      const gain = context.createGain();
